...
// src/client/ascii-motif.js
var CHARSET = " .,:\xB7-=*#\xB7>\xB7";
var FONT_SIZE2 = 13;
var LINE_HEIGHT = 15;
var TARGET_TEXT = "\xB7>\xB7";
function makeFont(weight) {
  return `${weight} ${FONT_SIZE2}px "Source Code Pro", monospace`;
}
...
var AsciiMotif = class {
  ...
  resize() {
    ...
    this.cols = Math.floor(w / (FONT_SIZE2 * 0.6));
...
// src/client/accordion.js
var BODY_FONT = '400 13px "Source Code Pro", monospace';
var LINE_HEIGHT_PX = 18.2;
...
// src/client/rich-text.js
var BODY_FONT2 = '400 13px "Source Code Pro", monospace';
var CHIP_FONT = '700 11px "Source Code Pro", monospace';
var CHIP_TONES = {
  lang: "lang",
  tool: "tool",
  role: "role",
...
function renderRichText(container, source) {
  ...
  const items = segments.map((seg) => {
    ...
    const prepared = prepareWithSegments(seg.content, BODY_FONT2);
...
// src/client/dynamic-layout.js
var BODY_FONT3 = '400 13px "Source Code Pro", monospace';
var LINE_HEIGHT2 = 18.2;
function layoutSection(section) {
  ...
  const widgetW = widget.clientWidth + 24;
  const widgetH = widget.clientHeight;
  const widgetLines = Math.ceil(widgetH / LINE_HEIGHT2);
  const text = textEl.dataset.content || textEl.textContent || "";
  const prepared = prepare(text, BODY_FONT3);
  const lineWidths = [];
  for (let i = 0; i < widgetLines + 2; i++) {
    lineWidths.push(sectionW - widgetW);
  }
  const fullWidthLine = sectionW;
  const narrowResult = layoutWithLines(prepared, sectionW - widgetW, LINE_HEIGHT2);
...
function init() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!btn) return;
    fetch("/api/gpg").then((r) => r.text()).then((key) => navigator.clipboard.writeText(key)).then(() => {
